...
Ti   = 5.8;      % Tempo médio em que o indivíduo em I permanece infeccioso
Tr   = 5.8;      % Tempo médio em que o indivíduo em R permanece infeccioso
f0   = 0.2131;    % Taxa inicial de notificação de casos
...
N    = 28.7538;     % Dias sem medidas de intervenção na dinâmica da epidemia
Nf   = length(t)+1; % Dias sem medidas de intervenção na contagem de casos
mu   = 0.011; % Parâmetro de ajuste da curva de taxa de transmissão
...
legend('População de infectados pre-sint./assint. I(t)', 'Casos sintomáticos ativos notificados R(t)', 'Casos sintomáticos ativos não-notificados U(t)',...
       'Cumulativo de casos notificados CR(t)', 'Cumulativo de casos não-notificados CU(t)', 'Notificações de casos no dia DR(t)','Location','NorthWest')
%legend('Modelo SIRU: CR','Modelo SIRU: CU','data')
%grid on;

% load('cumCasesCG.mat','cumCasesCG')
% 
% CRdata = cumCasesCG';
% plot(CRdata/plotScale,'-o');
